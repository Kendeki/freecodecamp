...
def calculate_demographic_data(print_data=True):
    ...
    rich_percentage = round(len(df[(num_min_workers) & (df["salary"] == ">50K")])/len(df[num_min_workers])*100, 1)

    # What country has the highest percentage of people that earn >50K?
    MASK = df.loc[df["salary"] == ">50K", "native-country"].value_counts()
    highest_earning_country = ((MASK)/(df["native-country"].value_counts())).idxmax()
    highest_earning_country_percentage = round(((MASK)/(df["native-country"].value_counts())).max()*100, 1)
